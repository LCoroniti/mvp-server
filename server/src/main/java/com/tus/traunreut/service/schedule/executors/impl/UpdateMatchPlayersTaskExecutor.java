...
public class UpdateMatchPlayersTaskExecutor implements ScheduledTaskExecutor<UpdateMatchPlayersTask> {
    ...
    public void execute(UpdateMatchPlayersTask task) {
        try {
            ...
            if (match.isEmpty()) {
                DB_LOG.error(Markers.DATABASE, "Cannot find match with ID {}", task.getMatchId());
                return;
            }
            ...
            if (players != null && !players.isEmpty()) {
                ...
                DB_LOG.info(Markers.DATABASE, "UpdateMatchPlayersTask executed successfully.");
            } else {
                DB_LOG.warn(Markers.DATABASE, "No player data available for the match. Retrying in 5 minutes...");
            }
        } catch (Exception e) {
            DB_LOG.error(Markers.DATABASE, "Error while executing UpdateMatchPlayersTask: ", e);
        }
    }
}